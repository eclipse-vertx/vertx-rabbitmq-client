...
A consumer can be configured to auto-ack messages, but can lead to message loss.
...
The Vertx-based consumer receives messages from RabbitMQ and adds them to a Vertx ReadStream from which they can be read.
This means that the messages will not be acked in the consumer directly and all messages available in the queue 
(up to the prefetch limit set on the channel or the maxInternalQueueSize set on the consumer)
will be placed into the ReadStream.
...
| Future publisher 2 with resend | 1.022s | 48923 M/s
| Future publisher 2 without resend | 0.873s | 57273 M/s
|===
