...
----
The client library will also attempt topology recovery as detailed in its documentation (this is enabled by default in the library and is not exposed in the vertx-rabbitmq-clientOptions).
...
This is easily enabled and works around one of the significant limitations of the client recover:
[source, java]
----
RabbitMQOptions options = new RabbitMQOptions();
options.setAutomaticRecoveryEnabled(false);
